...
def data_processing(arq_trips_sample, arq_drivers_sample):
    trips_table = pd.read_csv(arq_trips_sample, delimiter=';', encoding='utf-8') #Carregando a tabela de viagens

    drivers_table = pd.read_excel(arq_drivers_sample) #Carregando a tabela de motoristas

    df_triple = pd.DataFrame(trips_table) #criando um dataFrame

    df_drivers = pd.DataFrame(drivers_table) #criando um dataFrame

    df_triple['Data Viagem'] = pd.to_datetime(df_triple['Data Viagem'], errors='coerce', dayfirst=True) #Convertendo a coluna de data para o formato datetime64
    ...
    # Não sobrescrever merge_table com o resultado de inplace=True, que é None
